...
HRESULT PrintAttributes(IMFAttributes *pAttributes);
...
void PrintSampleInfo(IMFSample *pSample)
{
    ...
    DBGMSG(L"Sample - Ts: %I64d Dur: %I64d Len: %d\n", llTs, llDur, cbTotalLen);
    BYTE *pByte;
...
HRESULT PrintAttributes(IMFAttributes *pType)
{
    UINT32 count = 0;
...
LPCWSTR GetGUIDNameConst(const GUID& guid)
{
    ...
    IF_EQUAL_RETURN(guid, MFSampleExtension_DescrambleData);
    IF_EQUAL_RETURN(guid, MFSampleExtension_SampleKeyID);
    IF_EQUAL_RETURN(guid, MFSampleExtension_GenKeyFunc);
    IF_EQUAL_RETURN(guid, MFSampleExtension_GenKeyCtx);
    IF_EQUAL_RETURN(guid, MFSampleExtension_Encryption_SampleID);
    IF_EQUAL_RETURN(guid, MFSampleExtension_Encryption_KeyID);
